...
public class SortedHittablesFactory {
  ...
  private static final Function<AxisAlignedBoundingBox, Double> MIN_X =
    aabb -> aabb.getMin().getX();
  private static final Function<AxisAlignedBoundingBox, Double> MIN_Y =
    aabb -> aabb.getMin().getY();
  private static final Function<AxisAlignedBoundingBox, Double> MIN_Z =
    aabb -> aabb.getMin().getZ();
    ...
    switch ((int) (MathUtils.rand() * 3)) {
      ...
      default:
        throw new IllegalArgumentException(
          "Invalid random number when sorting hittables"
        );
    }
  ...
  private Comparator<SceneObject> getComparator(
